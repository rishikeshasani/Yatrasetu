 ...
 * mapped deterministically to each of the 25 sacred shrines (limited to exactly 2 hotels per shrine).
 */
export function getShrineAccommodations(siteId, siteName = '') {
  ...
  if (matches.length === 0 && cleanName) {
    matches = (HOTELS_50_DATA?.hotels || []).filter((h) => {
      const hSname = (h.site_name || '').toLowerCase();
      const hCity = (h.city || '').toLowerCase();
      const hName = (h.name || '').toLowerCase();
      return (
        hSname.includes(cleanName) ||
        cleanName.includes(hCity) ||
        cleanName.split(' ').some((tok) => tok.length > 3 && (hSname.includes(tok) || hName.includes(tok)))
      );
    });
  }
